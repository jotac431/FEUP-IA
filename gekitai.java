...
class Game{
  ...
  //heuristica para o número de 2 peças seguidas no tabuleiro de cada jogador
  /*public int setsOfTwoHeuristic(){
    //Lista de coordenadas ja visitadas que tem um adjacente
    List<Pair> marked = new ArrayList<>();
    int value=0;
    
    for(int i=0; i<board.length;i++){
      for(int j=0; j<board[0].length; j++){
          //se a peça existir, a casa seguinte tiver uma peça igual e a casa seguinte ainda não tiver sido visitada, então adicionar aos visitados e atualizar o valor
          if(pieceExist(i, j) && pieceExist(i, j+1) && board[i][j]==board[i][j+1] && !marked.contains(i,j+1)){
            marked.add(i,j);
            if(board[i][j]==1){
              value+=1;
            }else{
          ...
          if(pieceExist(i, j) && pieceExist(i+1, j) && board[i][j]==board[i+1][j] && !marked.contains(i+1,j)){
            marked.add(i,j);
            if(board[i][j]==1){
              value+=1;
            }else{
          ...
          if(pieceExist(i, j) && pieceExist(i+1, j+1) && board[i][j]==board[i+1][j+1] && !marked.contains(i+1,j+1)){
            marked.add(i,j);
            if(board[i][j]==1){
              value+=1;
            }else{
  ...
  //heuristica para o número de paças total no tabuleiro de cada jogador

  /*public int centerPiecesHeuristic(){
    int value=0;
    if(board[2][2]==1){
...
class MinMaxTree{
  ...
  public void expande(int depth, int n){
    if(depth>=0){
      for(int i =0;i<6;i++){
        for(int j = 0;j<6;j++){
  ...
  public void setHeuristic(int n){
    ...
    switch (n){
      case 0:
        heuristica += numberPiecesPlayerHeuristic();
      case 1:
        heuristica += numberPiecesDouble();
        break;
      case 2:
        heuristica += numberPiecesPlayerHeuristic()+ 2 * numberPiecesDouble();
        break;
      case 3:
        heuristica += valuePieceHeuristic();
        break;
      case 4:
...
public class gekitai{
  ...
  public static void ComputerVsComputer(Scanner keyboard){
    System.out.print("\033[H\033[2J");
    System.out.println("   GEKITAI\n");
    ...
    while(um.verifyWinner()==0){
      if(um.currentPlayer==1){
        ...
        dois.state.setCurrentPlayer(2);
        dois.setHeuristic(2);
        dois.expande(dificuldade, 2);
        List<Game> a = new ArrayList<>();
        for(int i=0;i<dois.nodeList.size();i++){
          if(dois.heuristica==dois.nodeList.get(i).heuristica){
            a.add(dois.nodeList.get(i).state);
            //um = dois.nodeList.get(i).state;
            //break;
          }
        }
        n = ran.nextInt(a.size());
        um = a.get(n);
        printGame(um);
        keyboard.nextLine();
      }
        ...
        printGame(um);
        System.out.println("\nJoga o jogador "+um.currentPlayer);
        tres = new MinMaxTree(um);
        tres.state.setCurrentPlayer(1);
        tres.setHeuristic(0);
        tres.expande(dificuldade, 0);
        List<Game> a = new ArrayList<>();
        for(int i=0;i<tres.nodeList.size();i++){
          if(tres.heuristica==tres.nodeList.get(i).heuristica){
            a.add(tres.nodeList.get(i).state);
            //um = tres.nodeList.get(i).state;
            //break;
          }
        }
        n = ran.nextInt(a.size());
